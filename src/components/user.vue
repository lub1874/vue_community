<template>
  <div class="warpper">
    <mu-appbar class="title" title="用户详情">
      <section class="user_msg">
        <img :src="user_msg_user.avatar_url" alt="avatar"/>
        ...
        <div class="msg">
          <mu-list>
            <mu-list-item v-if="user_msg.recent_topics" title="最近主题" toggleNested :open="false">
              <mu-icon class="icon" slot="left" value="insert_drive_file"></mu-icon>
              <mu-list-item
            ...
            <mu-list-item v-if="user_msg.recent_replies" title="最近回复" toggleNested :open="false">
              <mu-icon class="icon" slot="left" value="drafts"></mu-icon>
              <mu-list-item
                :to="{path:'/vuecommunitytest/content',query:{id:item.id}}"
                ...
                slot="nested"
                :title="item.title">
                <mu-icon class="icon" slot="left" value="insert_comment"></mu-icon>
              </mu-list-item>
              <span class="count">{{user_msg.recent_replies.length}}个</span>
            </mu-list-item>
            <mu-list-item v-if="user_msg.collect.topics" title="收藏主题" toggleNested :open="false">
              <mu-icon class="icon" slot="left" value="folder"></mu-icon>
              <mu-list-item
                :to="{path: '/vuecommunitytest/content', query: {id: item.id}}"
                ...
                slot="nested"
                :title="item.title">
                <mu-icon class="icon" slot="left" value="folder_open"></mu-icon>
              </mu-list-item>
              <span class="count">{{user_msg.collect_topics.length}}个</span>
            </mu-list-item>
          </mu-list>
        </div>
        <mu-raised-button @click="goBack" label="返 回" class="demo-raised-button" primary></mu-raised-button>
      </section>
    </mu-appbar>
  </div>
</template>
