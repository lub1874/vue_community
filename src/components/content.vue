<template>
  <div class="wrapper">
    <mu-appbar class="title" :title="data.title">
      <mu-icon-button @click="goBack" icon="arrow_back" slot="left"></mu-icon-button>
    </mu-appbar>
    <main class="main">
      <!-- 标题区 -->
      <header class="md_header">
        ...
        <mu-checkbox
          ...
          checkedIcon="favorite"></mu-checkbox>
        <div class="label">
        ...
        <li class="reply" v-for="(list,index) in data.replies" :key="list.id">
          <section class="msg">
            ...
            <div @click="open" v-if="accesstoken" class="ups">
              <mu-icon @click="like(index)" value="thumb_up" :size="16"></mu-icon>
              <span>{{list.ups.length}}</span>
              <mu-icon @click="changeReply(index)" class="textsms" value="textsms" :size="16"></mu-icon>
            </div>
          </section>
          ...
          <div v-if="accesstoken" class="reply_show" v-show="list.reply_show">
            ...
            <mu-raised-button @click="single_reply_content(index)" label="回复" class="demo-raised-button" primary></mu-raised-button>
            <mu-raised-button @click="exit_single_reply_content(index)" label="取消" class="demo-raised-button" primary></mu-raised-button>
          </div>
        </li>
      </ul>
      <ul v-if="accesstoken" class="replies">
        ...
        <li>
          ...
          <mu-raised-button @click="reply_content" label="回复" class="demo-raised-button" primary></mu-raised-button>
        </li>
      </ul>
    </main>
    <!-- 弹出提示框 -->
    <mu-dialog v-if="!accesstoken" :open="dialog" title="提示：" @close="close">
      请先登录，登录后即可点赞。
      <mu-flat-button slot="actions" @click="close" primary label="取消"></mu-flat-button>
      <mu-flat-button to="/vuecommunitytest/login" slot="actions" @click="close" primary label="确定"></mu-flat-button>
    </mu-dialog>
    <mu-dialog :open="isReply" title="提示：" @close="close">
      {{tips}}
            <mu-flat-button slot="actions" @click="close" primary label="确定"></mu-flat-button>
    </mu-dialog>
  </div>
</template>
<script>
  /*eslint-disable */
  import isheader from './common/header.vue'
  import Vue from 'vue'
